...
public class Analytics_v1 {
	...
	public static int getProjectCount(String userId) {
		int projectCount = 0;
		...
		try (Connection conn = DatabaseConnection_v1.getConnection();
			...
			stmt.setString(1, userId); // Setze die UserID als Parameter
			ResultSet rs = stmt.executeQuery();
			if (rs.next()) {
	...
	public static Object[][] getProjectNamesAndTaskCount(String userId) {
		...
		return DataPrep_v1.listMapInObject(projectList, keys);
	}

	// Abfrage ob zusätzliches TabbedPane angezeigt wird
	public static boolean isProjectLead(String userId) {
	...
	// mitarbeitet
	public static Object[][] getTaskInfo(String userId) {
		...
		return DataPrep_v1.listMapInObject(taskList, keys);
	}

	public static Object[][] getProjectTasks(String userId) {
		...
		return DataPrep_v1.listMapInObject(projectTasksList, keys);
	}
